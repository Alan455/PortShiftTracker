...
object DefaultCatalog {
    ...
    fun rules(): List<AllowanceRuleEntity> = buildList {
        ...
        add(fixed("AVV_FUORI_ORARIO_H", "FuoriOrario", 775, AllowanceCategory.AVVIAMENTO, 102))
        ...
        add(fixed("AVV_DS", "Donazione sangue", 9500, AllowanceCategory.ALTRE_VOCI, 512, basePayEffect = BasePayEffect.REPLACE_BASE, performanceMask = PerformanceType.TURNO.maskBit))
        add(fixed("AVV_INAIL", "Inail", 6780, AllowanceCategory.ALTRE_VOCI, 513, basePayEffect = BasePayEffect.REPLACE_BASE, performanceMask = PerformanceType.TURNO.maskBit))
        add(fixed("AVV_CONGEDO", "Congedo", 3000, AllowanceCategory.ALTRE_VOCI, 514, basePayEffect = BasePayEffect.REPLACE_BASE, performanceMask = PerformanceType.TURNO.maskBit))
